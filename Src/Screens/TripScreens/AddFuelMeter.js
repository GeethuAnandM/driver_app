...
const AddFuelMeter = (props) => {
  ...
  useEffect(() => {
    if (status == "start") {
      if (imageStore.fuelmeter.image == "") {
        setImage({
          path: "https://itac-qa-files.s3.ap-south-1.amazonaws.com/upload-icon-206311661704993232.png",
        });
      } else {
        setImage({
          path: imageStore.fuelmeter.image,
        });
      }
    } else if (status == "end") {
      if (imageStore.EndFuelmeter.image == "") {
        setImage({
          path: "https://itac-qa-files.s3.ap-south-1.amazonaws.com/upload-icon-206311661704993232.png",
        });
      } else {
        setImage({
          path: imageStore.EndFuelmeter.image,
        });
      }
    }
  }, [status]);
  ...
  const uplodeValue = async (checked) => {
    ...
    // props.navigation.navigate("AddSelfie", { status });
  };
    ...
    return (
      <React.Fragment>
        ...
        <Gradient_Button
          text="Next"
          //   active={active}
          onPress={() => {uploadImage();uplodeValue(checked)}}
        />
...
export default observer(AddFuelMeter);
