...
const StartFuel = (props) => {
  ...
  const { startFuel } = tripStore.selectedTrip;
  ...
  const moveNext = async () => {
    navigation.navigate("StartSelfie");
    setImageAdded(false);
  };
  const uplodeValue = async (checked) => {
    imageStore.setFuelmeterValue(checked);
    imageStore.setFuelValidated(true);
    setShowImage(false);
    await moveNext();
  };
  const ImageAdded = () => {
  ...
  const openCamera = () => {
    try {
      ...
      ImageCropPicker.openCamera({
        ...
        .then(async (image) => {
          setImage(image);
          imageStore.setFuelmeterImage(image.path);
          setShowImage(true);
        })
        .catch((err) => {
          AlertOpenSettings(err);
          if (imageStore.fuelmeter.FuelValidated == true) {
            setImageAdded(false);
          }
          console.log(err);
        });
  ...
  const uploadImage = async () => {
    ...
    await uploadImageURL({ ...image, screen: props.route.name }).then(
      async (res) => {
        if (res.message) {
          setUploaded(true);
          imageStore.setFuelmeterImage(res.message);
          logCustomEvent("Start Fuel Captured");
        }
      }
    );
  };
  ...
  // const skip = () => {
  //   props.navigation.navigate("StartSelfie");
  // };
  return (
    <Container>
        ...
        >
          ...
          >
            ...
            >
              <Image
                source={
                  imageStore.fuelmeter.image
                    ? isImage(imageStore.fuelmeter.image)
                    : isImage(image.path)
                }
            ...
            >
              {!showImage ? (
                <>
                  {/* only show Skip if user hasn’t started fuel & fuel isn’t validated */}
                  {!startFuel && !imageStore.fuelmeter.FuelValidated && (
                    <TouchableOpacity
              ...
              {imageStore.fuelmeter.FuelValidated && !imageAdded && (
...
export default observer(StartFuel);
